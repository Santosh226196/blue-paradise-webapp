...
export function BillingPage() {
  ...
  const { data: preselectedCustomer } = useGetCustomerQuery(customerId!, {
    skip: !customerId,
  });
  ...
  function handlePay() {
    ...
    createTransaction({
      ...
      .catch((err) => {
        showToast(
          "error",
          err?.data?.message || "Payment failed. Please try again.",
        );
        setErrorModal({
          open: true,
          message:
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="lg:grid lg:grid-cols-5 lg:gap-6">
        {/* Main Column */}
        <div
          className={`space-y-4 ${step < 2 ? "lg:col-span-3" : "lg:col-span-5"}`}
        >
                ...
                <div className="space-y-2">
                  {customers?.map((c, i) => (
                    <button
                      ...
                      className="liquid-glass relative overflow-hidden w-full p-3.5 text-left transition-all duration-200 min-h-14 active:scale-[0.99] animate-fade-up flex items-center justify-between border border-white/10 hover:border-cyan-400/40 cursor-pointer"
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fade-up">
              {selectedCustomer && (
                <GlassCard
                  padding={false}
                  className="p-4 flex items-center justify-between border-cyan-400/30"
                >
                  ...
                  <button
                    onClick={() => setStep(0)}
                    className="text-xs font-bold px-3 py-1.5 rounded-lg border border-cyan-400/30 text-cyan-300 hover:bg-cyan-400/10 cursor-pointer"
                  >
                    Change
                  </button>
                </GlassCard>
        ...
        {step < 2 && (
          <div className="hidden lg:block lg:col-span-2 space-y-4">
            <GlassCard>
              <h3
                className="text-xs font-bold uppercase tracking-wider mb-4 text-fg-muted"
              >
                Quick Summary
              </h3>
