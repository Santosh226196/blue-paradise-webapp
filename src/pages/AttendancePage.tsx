...
export function AttendancePage() {
  ...
  const { data: customers } = useGetCustomersQuery(
    { search },
    { skip: !showCheckIn && search.length < 2 },
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <GlassCard padding={false} className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p
                className="text-2xl font-bold font-mono text-fg"
              >
                {currentlyInPool.length}
              </p>
        ...
        <GlassCard className="animate-scale-in border-cyan-400/30">
          ...
          <div className="space-y-4">
            {!selectedCustomer ? (
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-1">
                    {customers?.slice(0, 5).map((c) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 rounded-xl text-sm transition-all min-h-11 flex items-center justify-between cursor-pointer"
                        style={{
              ...
              <>
                <div className="flex items-center justify-between p-4 rounded-xl border border-cyan-400/40 bg-cyan-400/10">
                  <div className="flex items-center gap-3">
                    {selectedCustomer.photoUrl || checkInPhoto ? (
                      <img
                        src={checkInPhoto || selectedCustomer.photoUrl}
                        alt=""
                ...
                <div className="p-3.5 rounded-xl border border-white/10 bg-black/20 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400">
                        {checkInPhoto
                          ? "Photo verified for entry"
                          : "Snap live photo at gate (optional)"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-cyan-400/20 border border-cyan-400 text-cyan-300 hover:bg-cyan-400/30 flex items-center gap-1 cursor-pointer"
                    >
                      <IoCamera size={14} />
                ...
                <div className="space-y-2">
                  <Input
                    label="Lane Allocation (Optional)"
                    type="number"
                    min={1}
                    max={10}
                    value={lane}
      ...
      <GlassCard>
        <h2
          className="font-display text-lg font-bold mb-4 text-fg"
        >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-mono text-slate-400">
                      In at {formatTime(r.checkInTime)} ·{" "}
                      {visitTypeLabels[r.visitType] ?? r.visitType}
                      {r.lane ? ` · Lane ${r.lane}` : ""}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  {r.photoUrl ? (
                    <img
                      ...
                      alt=""
                      loading="lazy"
                      className="w-9 h-9 rounded-lg object-cover border border-slate-600 opacity-80"
                    />
                  ) : (
