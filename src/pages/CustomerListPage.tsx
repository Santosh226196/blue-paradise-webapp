      ...
      <div className="flex gap-2 flex-wrap">
        {filterChips.map(({ key, label }) => (
          <button
            ...
            className="px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 min-h-9 cursor-pointer"
            style={{
              background:
                activeFilter === key ? "var(--glow-aqua)" : "var(--glass-bg)",
        ...
        <div className="space-y-2">
          {customers?.map((customer, i) => (
            <Link
              ...
              className="liquid-glass relative overflow-hidden flex items-center justify-between p-3.5 sm:p-4 transition-all duration-200 min-h-14.5 animate-fade-up group border border-white/10 hover:border-cyan-400/40 cursor-pointer"
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <IoChevronForward
                  size={18}
                  className="text-fg-muted transition-transform duration-200 group-hover:translate-x-1 group-hover:text-cyan-300"
                />
              </div>
            </Link>
          ))}
