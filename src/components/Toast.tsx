...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-semibold flex-1" style={{ color: c.text }}>{toast.message}</p>
      <button onClick={onDismiss} className="text-xs font-bold shrink-0" style={{ color: "var(--text-muted)" }}>✕</button>
    </div>
  );
}
